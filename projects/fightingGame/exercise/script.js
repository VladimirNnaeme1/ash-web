...
class Player {
  ...
  heal (player) {
    ...
    return `${player.name} heals for the ${hpAmount} HP`
...
let Vlad = new Player('Vlad',100,10)
let Clint = new Player('Clint', 100, 10)
...
document.addEventListener('keydown', function(e) {
  ...
  if(e.key == 'q' && p2.health <= 100 && game.isOver == false){
    p1.strike(p1,p2,p1.attackDmg)
...
// ** Player 2 Controls **
document.addEventListener('keydown', function(e) {
    if(e.key == 'p' && p1.health <= 100 && game.isOver == false){
      p2.strike(p2,p1,p2.attackDmg)
...
console.log(p1.strike(p1,p2,game.isOver))